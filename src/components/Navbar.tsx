  ...
  return (
    <header className="sticky top-0 z-40 w-full border-b border-border bg-card/95 backdrop-blur supports-backdrop-filter:bg-card/80">
          ...
          <div className="flex gap-6">
            {mainLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="text-sm font-medium text-primaryText transition-all border-b-2 border-transparent hover:border-accent hover:text-accent pb-1"
              >
                {link.label}
              </Link>
            ))}
      ...
      <nav className="mx-auto flex max-w-7xl items-center justify-center px-4 py-2 sm:px-6 lg:px-8">
        ...
        <button
          type="button"
          className="rounded-lg p-2 text-primaryText transition-colors hover:bg-highlight hover:text-accent focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent/40 lg:hidden"
            ...
            <div className="mb-4 flex flex-col gap-1 border-b border-border pb-4">
              ...
              {mainLinks.map((link) => (
                <Link
                  key={link.href}
                  href={link.href}
                  className="rounded-lg px-4 py-3 font-medium text-primaryText transition-colors hover:bg-highlight hover:text-accent"
              ...
              {categories.map((category) => (
                <div key={category.slug} className="space-y-1">
                  <Link
                    ...
                    className="block rounded-lg px-4 py-3 font-medium text-primaryText transition-colors hover:bg-highlight hover:text-accent"
                    onClick={() => setMobileMenuOpen(false)}
                  >
                    {category.name}
                  </Link>
                  <div className="ml-4 flex flex-col gap-1">
                    {category.subcategories.map((sub) => (
                      <Link
                        ...
                        className="rounded-lg px-4 py-2 text-sm text-secondaryText transition-colors hover:bg-highlight hover:text-accent"
                        onClick={() => setMobileMenuOpen(false)}
                      >
                        {sub.name}
                      </Link>
